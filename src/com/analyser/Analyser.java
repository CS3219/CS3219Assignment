...
public class Analyser {
	...
	public ArrayList<ArrayList<String>> analyse(JobDescObject jobDesc,
			ArrayList<CVObject> cvs, String position) {
		...
		for (int i = 0; i < cvs.size(); i++) {
			...
			minExpScore = compareExp(minWorkExp, cvExp, cvProj, cvPublication,
					position);
			// extraExpScore = compareExp(extraWorkExp, cvExp, cvProj,
			// cvPublication, position);

			if (minEduScore == -1 && minSkillScore > 0.5) {
				minEduScore = 1;
			}
			// System.out.println("minEduScore = "+minEduScore+
			// " minExpScore = "+minExpScore+ " minSkillScore = "+minSkillScore
			// + " extraExpScore = "+extraExpScore);
			// if one min req is unfulfilled, score = 0
			// if((minWorkExp.size() > 0 && minExpScore == 0.0) ||
			// (minEduReq.size() > 0 && minEduScore == 0.0) ||
			// (minSkillReq.size() > 0 && minSkillScore == 0.0)) {
			// score = 0.0;
			// } else {
			score = (0.3 * minSkillScore + 0.3 * minExpScore + 0.3
					* minEduScore + 0.1 * (extraExpScore + extraSkillScore)) * 100;
			// }
			// System.out.println("score before decimal = " +score);
			// String result = String.format("%.2f", value);
			// Double.toString(score)
			ResultDetail resultDetail = new ResultDetail(cvs.get(i).getName(),
					String.format("%.2f", score), score);
	...
	private double compareExp(ArrayList<ParsedObject> workExp,
			ArrayList<ExpObject> cvExp, ArrayList<ExpObject> cvProj,
			ArrayList<ExpObject> cvPublication, String position) {
		...
		Pattern patternForSimilar = Pattern
				.compile("\\bsimilar\\b|\\bequivalent\\b|\\brelevant\\b|\\brelated\\b|\\binterrelated\\b|\\bsame\\b");

		for (int i = 0; i < workExp.size(); i++) {
			// System.out.println("workExp = " +
			// workExp.get(i).getWords().get(0));
			temp = workExp.get(i).getWords().get(0)
					.replaceAll("\\d+(.|)\\d+", "");
			...
			for (int k = 1; k < workExp.get(i).getWords().size(); k++) {
				Matcher matcher = patternForSimilar.matcher(workExp.get(i)
						.getWords().get(k));
				String[] tempForComma = workExp.get(i).getWords().get(k)
						.split(",");
				...
				workExp.get(i)
						.getWords()
						.set(k,
								workExp.get(i).getWords().get(k)
										.replaceAll("\\s+", " "));
				workExp.get(i).getWords()
						.set(k, workExp.get(i).getWords().get(k).trim());
				...
				for (int j = 0; j < cvExp.size(); j++) {
					...
					// "");
					// System.out.println("cv's size = " +
					// cvExp.get(j).getDuration());
					for (int y = 0; y < cvExp.get(j).getDescription().size(); y++) {
						// System.out.println("job desc = " +
						// workExp.get(i).getWords().get(k) + " cv = " +
						// cvExp.get(j).getDescription().get(y));
						word = workExp.get(i).getWords().get(k).split("\\s");

						for (int p = 0; p < word.length; p++) {
							// System.out.println("word[p] = "+ word[p] +
							// " p = " + p);
									...
									&& Pattern
											...
											.matcher(
													cvExp.get(j)
															.getDescription()
															.get(y)).find()) {
								// cvExp.get(j).getDescription().get(y).contains(word[p])
								// || (matcher.find() &&
								// cvExp.get(j).getDescription().contains(position)))
								// {
								count++;
								// System.out.println("count = "+ count);
							}
							// else if(word[p].contains("ment") ||
							// word[p].contains("tion")) {
							// count++;
							// System.out.println("count = "+ count );
							// }
	...
	private double compareEdu(ArrayList<ParsedObject> eduReq,
			ArrayList<String> cvEducation) {
		int eduNum = 0;
		boolean isSimilar = false;
		Pattern patternForDiploma = Pattern
				.compile("\\bbachelor\\b|\\bmaster\\b|\\bphd\\b|\\bdiploma\\b");
		Pattern patternForBE = Pattern
				.compile("\\bbachelor\\b|\\bmaster\\b|\\bphd\\b");
		...
		Matcher matcher;
		String prev = "";
		// System.out.println("eduReq'size = " + eduReq.size() +
		// " cvEducation.size() = " + cvEducation.size());
		for (int i = 0; i < eduReq.size(); i++) {
			for (int k = 0; k < eduReq.get(i).getWords().size(); k++) {
				matcher = patternForSimilar.matcher(eduReq.get(i).getWords()
						.get(k));
				eduReq.get(i)
						.getWords()
						.set(k,
								eduReq.get(i)
										...
										.get(k)
										.replaceAll(
												"\\bgraduate\\b|\\bgraduated\\b|\\bdegree(s|)\\b",
												""));
				eduReq.get(i)
						.getWords()
						.set(k,
								eduReq.get(i)
										...
										.get(k)
										.replaceAll(
												"\\bbe\\b|\\bbachelors\\b|\\bbs\\b|\\bundergraduate\\b",
												"bachelor"));
				eduReq.get(i)
						.getWords()
						.set(k,
								eduReq.get(i)
										...
										.get(k)
										.replaceAll(
												"\\bmsc\\b|\\bmasters\\b|\\bms\\b|\\bpostgraduate\\b",
												"master"));
				eduReq.get(i)
						.getWords()
						.set(k,
								eduReq.get(i)
										...
										.get(k)
										.replaceAll(
												"\\bdiploma\\b|\\bdiplomas\\b|\\bpecialist diploma\\b",
												"diploma"));
				eduReq.get(i)
						.getWords()
						.set(k,
								eduReq.get(i)
										...
										.get(k)
										.replaceAll(
												"\\bphd\\b|\\bph.d\\b|\\bdoctorate(s|)\\b|\\bdoctor(s|)\\b|\\bDoctor of Philosophy\\b",
												"phd"));
				...
				for (int j = 0; j < cvEducation.size(); j++) {
					...
					// System.out.println("cv = " + cvEducation.get(j)
					// +" job desc = " + eduReq.get(i).getWords().get(k));
					if (Pattern.compile("\\bbachelor\\b")
							.matcher(eduReq.get(i).getWords().get(k)).find()) {
						...
						if (matcher.find()) {
							...
							cvEducation
									.set(j,
											cvEducation
													.get(j)
													.replaceAll(
															"\\bbachelor\\b|\\bmaster\\b|\\bphd\\b",
															""));

							eduReq.get(i)
									.getWords()
									.set(k,
											eduReq.get(i).getWords().get(k)
													.replaceAll("\\s+", " "));
							cvEducation.set(j,
									cvEducation.get(j).replaceAll("\\s+", " "));
							...
							// System.out.println("inCVs = " +
							// cvEducation.get(j) + " injob desc = " +
							// eduReq.get(i).getWords().get(k));
							if (Pattern
									.compile(
											"\\b"
													+ eduReq.get(i).getWords()
															.get(k) + "(s|)\\b")
									.matcher(cvEducation.get(j)).find()) {
					...
					} else if (Pattern.compile("\\bmaster\\b")
							.matcher(eduReq.get(i).getWords().get(k)).find()) {
							...
							eduReq.get(i)
									...
									.set(k,
											eduReq.get(i)
													.getWords()
													.get(k)
													.replaceAll("\\bmaster\\b",
															""));
							cvEducation.set(
									j,
									cvEducation.get(j).replaceAll(
											"\\bmaster\\b|\\bphd\\b", ""));
							...
							eduReq.get(i)
									...
									.set(k,
											eduReq.get(i).getWords().get(k)
													.trim());
							cvEducation.set(j, cvEducation.get(j).trim());
							// remind parser to split by "or" "," when bachelor
							// of comp sci or related field in diff index
							if (Pattern
									.compile(
											"\\b"
													+ eduReq.get(i).getWords()
					...
					} else if (Pattern.compile("\\bphd\\b")
							.matcher(eduReq.get(i).getWords().get(k)).find()) {
					...
					} else if (Pattern.compile("\\bdiploma\\b")
							.matcher(eduReq.get(i).getWords().get(k)).find()) {
							...
							cvEducation
									.set(j,
											cvEducation
													.get(j)
													.replaceAll(
															"\\bbachelor\\b|\\bmaster\\b|\\bphd\\b|\\bdiploma\\b",
															""));
						...
						eduReq.get(i).getWords()
								.set(k, eduReq.get(i).getWords().get(k).trim());
						cvEducation.set(j, cvEducation.get(j).trim());

						// System.out.println("prev = " +prev + " inCVs = " +
						// cvEducation.get(j) + " injob desc = " +
						// eduReq.get(i).getWords().get(k));
						if (Pattern
								.compile(
										"\\b" + eduReq.get(i).getWords().get(k)
												+ "(s|)\\b")
								.matcher(cvEducation.get(j)).find()) {
	...
	private double compareSkill(ArrayList<ParsedObject> skillReq,
		...
		for (int i = 0; i < skillReq.size(); i++) {
			...
			for (int k = 0; k < skillReq.get(i).getWords().size(); k++) {
				...
				skillReq.get(i)
						.getWords()
						.set(k,
								bypassUnwanted(skillReq.get(i).getWords()
										.get(k)));
				skillReq.get(i).getWords()
						.set(k, skillReq.get(i).getWords().get(k).trim());
				skillReq.get(i)
						.getWords()
						.set(k,
								skillReq.get(i).getWords().get(k)
										.replaceAll("\\s+", " "));
				// if(!skillReq.get(i).getWords().get(k).equals("")){
				// skillReq.get(i).getWords().set(k,
				// skillReq.get(i).getWords().get(k).substring(0,
				// skillReq.get(i).getWords().get(k).length()-1));
				// }
				// System.out.println("JD's skill = " +
				// skillReq.get(i).getWords().get(k));
				...
				for (int j = 0; j < cvSkill.size(); j++) {
					...
					if (!cvSkill.get(j).equals("")
							&& cvSkill.get(j).endsWith(" ")) {
						cvSkill.set(
								j,
								cvSkill.get(j).substring(0,
										cvSkill.get(j).length() - 1));
					}
					cvSkill.set(j, cvSkill.get(j).trim());
					cvSkill.set(j, cvSkill.get(j).replaceAll("\\s+", " "));

					String patternString = "\\b" + cvSkill.get(j) + "(s|)\\b";
					// System.out.println("cv = " + cvSkill.get(j) +
					// " JD's skill = " + skillReq.get(i).getWords().get(k));
					...
					if (matcher.find() && !cvSkill.get(j).equals("")) {
						...
						skillNum++;
						// cvSkill.set(j, "");
						// System.out.println("skillnum = " + skillNum +" cv = "
						// + cvSkill.get(j)+ " JD's skill = " +
						// skillReq.get(i).getWords().get(k));
					}
				}
			}
	...
	private String bypassUnwanted(String word) {
		word = word
				.replaceAll(
						"\\b(strong |deep |good |excellence |great |excellent |)(proficiency\\b|specialist domains\\b|technical knowledge\\b|knowledge\\b|expertise\\b|ability\\b|foundation\\b|experience\\b)",
						"");
		return word;
	}
}
